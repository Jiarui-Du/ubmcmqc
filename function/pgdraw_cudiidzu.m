function X = pgdraw_cudiidzu(u1,u2,z)
% X = pgdraw_cudiidzu() generates random variates from PG(1, z) by unifrom variates
...
Ixe = ~Isampled & (u1 < ratio);
X(Ixe) = t-log(1-u2(Ixe))./K(Ixe);
%% Sampled from truncated inverse-gaussian, as required
Ixig = ~Isampled & ~(u1 < ratio);
% err = 1e-14;
% sampleX = drawg(u,t,mu,lambda,n,Ixig,err);
% sampleX = trc_drawinvgauss(u2(Ixig),t,mu(Ixig),lambda,err);
sampleX = truncinvgrng_vec(z(Ixig),t,u2(Ixig));
X(Ixig) = sampleX;
%% Step 2: Iteratively calculate Sn(X|z), starting at S1(X|z), until U <= Sn(X|z) for an odd n or U > Sn(X|z) for an even n
    ...
    while(~all(Idone))
        ...
        X(Ixre) = X(Ixre);
        u1(Ixre) = rand(sum(Ixre),1);
        u2(Ixre) = rand(sum(Ixre),1);
        Idone(Ixre) = true;
        even = ~even;
        sign = -sign;
        i = i + 1;
